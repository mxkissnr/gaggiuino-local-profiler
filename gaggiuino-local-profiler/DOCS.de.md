...
Der Editor deckt Name, Wassertemperatur, Rezept (Dosis/Ausbeute/Ratio) und die Phasen eines Profils ab — jede Phase hat einen Namen, Typ (Fluss/Druck/Manuell), ein Ziel (Start/Ende/Kurve/Zeit), eine Fluss-/Druck-Restriktion und Stopp-Bedingungen (Zeit, Druck-/Fluss-Schwellenwerte, Gewicht, gefördertes Wasser). Phasen können hinzugefügt, entfernt und neu geordnet werden; eine Live-Vorschau-Chart zeichnet sich beim Bearbeiten neu, indem sie aus den Phasen-Kurven eine Zeitreihe synthetisiert, so wie die Maschine sie selbst durchlaufen würde.
...
Jede Bohne mit Geschmacks-Tags zeigt in der Bibliothek einen 🎡-Button. Er öffnet ein Sunburst-Diagramm der Kaffee-Aroma-Hierarchie — die Kategorienstruktur folgt dem SCA/WCR *Coffee Taster's Flavor Wheel* (2016); dies sind eigens abgeleitete Daten, übersetzt in alle 6 UI-Sprachen (DE, EN, IT, FR, ES, NL), nicht die Original-Grafik. Die Aromen aus den Tags der Bohne werden gegen das Rad gematcht (exakter Label-Treffer in jeder der 6 Sprachen, eine deutsche Alias-Tabelle für zusammengesetzte/umgangssprachliche Begriffe, dann Wortgrenzen-Textcontainment). Nur der Treffer-Pfad — die tatsächlichen Aromen der Bohne plus ihre übergeordneten Kategorien — wird groß und vollfarbig gezeigt, alles andere bleibt ein schmaler, entsättigter, unbeschrifteter Streifen — das komplette Rad bleibt als Kontext sichtbar, ohne mit den eigentlichen Treffern um Aufmerksamkeit zu konkurrieren. Die 9 Top-Kategorien beschriften sich weiterhin direkt im eigenen (breiten, praktisch nie kollidierenden) Segment; Treffer im mittleren und äußeren Ring wandern stattdessen nach außen über den Ringrand hinaus, jeweils mit einem kleinen farbigen Quadrat vor dem Text, das exakt die Segmentfarbe hat, statt einer Verbindungslinie zum Segment — dieselbe Konvention wie beim echten SCA/WCR-Poster — inklusive automatischer Kollisionsvermeidung, damit jede Kombination gleichzeitig aktiver Aromen lesbar bleibt, egal wie eng ihre Segmente beieinanderliegen. Nicht zugeordnete Aromen werden als einfache Chips unter dem Diagramm gelistet, damit nichts stillschweigend verloren geht. Das Rad-Modal scrollt nie — es passt sich immer an den Viewport an und lässt sich über das ✕ oder per Klick/Tap außerhalb schließen.
