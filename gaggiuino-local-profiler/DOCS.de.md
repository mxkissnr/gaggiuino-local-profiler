...
Am Desktop gibt es drei Spalten: eine einklappbare **Navigations-Rail** ganz links (Shots / Live / Bibliothek / Statistiken / Bezugslog / Wartung / Bestellungen, ein Trenner, dann Einstellungen — jeweils mit Icon), das Shot-Listen-Panel und den Inhaltsbereich. Die Rail lässt sich über den Umschalter am oberen Rand auf Icon-only zusammenklappen (Zustand in `localStorage` gespeichert, beim Neuladen wiederhergestellt); das Shot-Listen-Panel hat einen eigenen, davon unabhängigen Einklapp-Button.

Auf schmalen Bildschirmen (Smartphones, Tablets im Hochformat) wird die Rail komplett ausgeblendet, stattdessen übernimmt eine **Bottom-Navigation**: Shots / Live / Bibliothek / Statistiken plus ein „Mehr"-Sheet mit Bezugslog / Wartung / Einstellungen / Bestellungen — jeder Eintrag hat ein Icon, keine Emoji mehr in der Navigation. Die Shot-Liste ist der primäre „Shots"-Screen, kein Drawer/Overlay: ein Tap auf einen Shot wechselt zur Detailansicht, ein Zurück-Pfeil in der Kopfzeile führt zur Liste zurück. Der Multi-Maschinen-Umschalter sitzt auf jeder Bildschirmgröße in einer schmalen Leiste über dem Inhaltsbereich (nicht mehr auf einer horizontalen Tab-Leiste). Bedienelemente, die am Desktop erst bei Hover erscheinen (z. B. der Vergleichen-Button in der Shotliste, die Sternebewertung), bekommen auf Touch-Geräten ein volles 44×44px-Tapziel. Zwei Dinge sind noch nicht in die mobile Ansicht übernommen: Badge-Zähler (Wartung/Bestellungen) erscheinen nicht im „Mehr"-Sheet, nur die Sichtbarkeit der Einträge ist genauso gesteuert wie am Desktop; und die pulsierende „Shot läuft"-Anzeige des Live-Tabs wird nicht auf das Bottom-Nav-Icon übertragen, das nur ein-/ausgeblendet wird.
...
1. **Generisches Shopify** — jeder Shopify-Shop stellt einen Endpunkt `<Produkt-URL>/products/<handle>.js` bereit; passt die URL, wird dessen JSON genauso geparst wie bei den eingebauten Shopify-Shops (Name, Rösterei, aus der Beschreibung abgeleitete Aromen/Herkünfte, Bild, Preis, Größenvarianten). Das kuratierte Aroma-Vokabular umfasst jetzt mehr Specialty-/englische Begriffe (Lychee, tropisch, milchig-säuerlich, Guave, Passionsfrucht, Papaya, zusätzlich zu Mango) und wird unabhängig von einer bestimmten Überschrift erkannt, sodass Shops ohne eine elbgold-artige "Sensorik/Geschmack"-Überschrift trotzdem Aroma-Tags bekommen. Liest sich der `vendor`-Wert des Shops nicht wie ein echter Rösterei-Name (z. B. wenn ein Shop das Feld für ein Geschmacksprofil-Tag zweckentfremdet), fällt das Rösterei-Feld auf die eigene Domain des Shops zurück.
