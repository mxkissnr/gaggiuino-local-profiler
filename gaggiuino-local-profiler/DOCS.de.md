...
| Wasserstand | ✅ | ✅ mit ALBA-Sensor (opt-in im Maschinenformular) |
| Profil lesen | ✅ | ✅ |
| Profil erstellen/ändern/löschen | ✅ | ✅ (Standard + Pro, direkt auf der Maschine gespeichert) |
...
Hat die **aktive Maschine** (die im Topbar-Umschalter gewählte, ohne explizite Auswahl die Standardmaschine) ein Farbthema gesetzt, bestimmt dieses Farbthema auch die **Akzentfarbe der gesamten App** — der "Farbschema"-Farbwähler in den Einstellungen wird dadurch überall dort überschrieben, wo er greift (Buttons, Hervorhebungen, aktive Zustände), und aktualisiert sich sofort, wenn die aktive Maschine gewechselt oder ihr Farbthema bearbeitet wird — kein Neuladen nötig. Das ist eine reine Ergänzung: Ohne gesetztes Farbthema an der aktiven Maschine funktioniert der Farbwähler wie gewohnt, und seine Farbfelder bleiben auch bei aktiver Überschreibung klickbar (die Auswahl wird für später gespeichert und als Fallback-Akzentfarbe verwendet) — sie werden nur abgedunkelt mit einem kurzen Hinweis dargestellt, warum sie gerade keine sichtbare Wirkung haben.
...
Unabhängig vom Hell/Dunkel-Umschalter bietet ⚙ Einstellungen → Farbschema jetzt dieselben 8 benannten Presets (Amber Americano, Ruby Ristretto, Copper Cortado, Twilight Turkish, Marbled Macchiato, Ember Espresso, Mulberry Mocha, Frosty Flat White) wie der Farbthema-Wähler pro Maschine (siehe [Multi-Maschinen-Modus](#multi-maschinen-modus-v200)) — eine gemeinsame Preset-Liste steuert beide. Die App-Akzentfarbe folgt jetzt der jeweils aktiven Maschine im Topbar-Umschalter: Hat diese Maschine ein eigenes Farbthema gesetzt, überschreibt es diesen Farbwähler live (aktualisiert sich sofort beim Wechseln, kein Neuladen nötig), mit der eigenen Auswahl hier als Fallback, sobald die aktive Maschine kein eigenes Farbthema hat — dasselbe additive Verhalten wie zuvor, nur jetzt bezogen auf die aktive statt immer die Standardmaschine. Eine bestehende Auswahl aus dem alten 6-Farben-Set (Amber/Ocean/Aurora/Ember/Forest/Crema) wird beim ersten Laden automatisch auf das nächstliegende neue Preset migriert. Cremas alter Sonderfall — die neutrale Grau-Skala selbst warm einzufärben und das mit einer mitgelieferten Serifenschrift für Bohnennamen zu kombinieren — entfällt; Fraunces (SIL Open Font License, lokal ausgeliefert, kein externes Schrift-CDN) wird jetzt unabhängig vom Akzent-Theme für Bohnennamen in der Bibliothek-Ansicht und auf der teilbaren Shot-Karte verwendet, alle numerischen/Daten-Anzeigen bleiben in der regulären serifenlosen Schrift. Für die 8 neuen Presets ist inzwischen auch der Hell-Theme-Textkontrast-Durchgang nachgeholt: Wo die rohe Preset-Farbe als Text (z. B. Sterne-Bewertung, aktive Tabs, Sortierpfeile) auf den vier hellen Oberflächen unter 4,5:1 (WCAG AA) fiel, greift im Hell-Theme automatisch eine entlang desselben Farbtons abgedunkelte Variante — die eigentliche Akzentfarbe (Verlauf, Glow) bleibt davon unberührt.
