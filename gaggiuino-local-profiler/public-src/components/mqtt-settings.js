...
let _discovery = null;
// #1050: GET /api/mqtt/settings no longer echoes the stored broker password
// back (it reports `hasPassword` instead), so the form cannot pre-fill the
// field. This tracks whether one is stored, so saveMqttSettings() can tell
// "the user left it alone" (omit the key -> backend keeps the stored value)
// apart from "there was never one" (send the empty string).
