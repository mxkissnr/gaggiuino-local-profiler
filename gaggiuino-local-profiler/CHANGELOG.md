## [Unreleased]
### Fixed
- **The Settings → Farbschema accent, when used as text (e.g. the star rating, active tabs, sort arrows), is now readable in the light theme for all 8 named presets.** #1019 shipped that picker without a light-theme contrast audit; amber-americano, copper-cortado, twilight-turkish, marbled-macchiato, ember-espresso and frosty-flat-white measured as low as 1.72:1 against the light theme's surfaces (well under the WCAG AA 4.5:1 floor) — each now gets a darkened, same-hue override applied automatically in the light theme, same audit method used for the old 6-accent set (#811). The accent's actual fill, gradient and glow are untouched. Closes #1021
