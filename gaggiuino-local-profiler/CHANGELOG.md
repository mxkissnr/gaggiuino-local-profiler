...
- **The `debug_logging` add-on option now takes effect on the Go backend** (sync/poll/machine-registry/import diagnostic detail), matching what it already did on the previous Node backend. Off by default — no visible change unless you turn it on. Closes #977
