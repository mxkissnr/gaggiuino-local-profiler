...
- **GaggiMate adapter (experimental).** `lib/machines/gaggimate/` — own client implementation written from GaggiMate's public WebSocket protocol description, no code or assets vendored from the GaggiMate repository (GaggiMate is CC BY-NC-SA licensed, same project-boundary rule as Gaggiuino: interoperate as a client, never embed). Covers live status over the `/ws` JSON WebSocket, `.slog`/`index.bin` binary shot-history parsing, and profile CRUD. GaggiMate's API has no brew start/stop, so that capability is hidden via the adapter's capability flags. **This has not been tested against real GaggiMate hardware** — only against the protocol spec and (recommended before relying on it) GaggiMate's own official desktop simulator. Treat GaggiMate support as experimental until confirmed working on real hardware. `lib/machines/gaggimate/`, `test/gaggimate-*.test.js`.
