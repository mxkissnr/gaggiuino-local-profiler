...
### Added
- **The Live tab (telemetry chart + preheat widget) and the sidebar's shot counter now also update in real time over the `GET /api/events` SSE connection**, instead of only their existing 1s/10s/30s polling. `lib/poll.js`'s 1s status poll now broadcasts a `LIVE_SNAPSHOT` after every tick (success and error/unreachable alike — same shape `GET /api/live/data` already returns, via a new shared `buildLiveDataResponse()`), and `lib/preheat.js`/`lib/poll.js` broadcast a `PREHEAT_UPDATE` (same shape `GET /api/preheat` returns) both on the existing 30s watcher tick and immediately at every state change that used to only be visible on the next tick or poll (switch on/off, the ready-by target being set/cleared/fired, and the temperature-stable flip). New SSE connections are also primed with the current preheat snapshot, so the Ready badge doesn't wait up to 30s for its first update. The Live view's own `fetchLiveData()`/`fetchPreheatData()` polling intervals are now skipped entirely while SSE is active, falling back to the previous polling exactly as before whenever it isn't (same `S.sseActive` convention #735 established). Separately, the sidebar's shot counter and flap-board odometer now advance live during a backfill (`baseline + current`, from the existing `SYNC_PROGRESS` push) instead of only jumping once the import finishes and `loadData()` reloads everything — the exact DB count is still reconciled via `loadData()` on a successful `SYNC_COMPLETE`, correcting any drift. All three REST endpoints (`/api/live/data`, `/api/preheat`, `/api/status`) are unchanged. Closes #736, #742
