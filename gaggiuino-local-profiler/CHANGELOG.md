...
### Added
- **Settings → Machines' "Test connection" button now saves the machine first, then tests it** — a brand-new (not-yet-saved) machine can be tested with one click instead of hitting a "save first" error. On success the result is shown inline briefly, then the form closes and the machines list reloads automatically, matching the plain "Save" button's behavior. The separate "Save & test" button introduced in an earlier unreleased change is gone again — its "leave the form open" behavior turned out to be the wrong default once tried live. Closes #729
- **Saving any machine (not just a default-host change) now triggers a catch-up shot sync for it.** Previously, only a default machine's host change kicked off `syncShots()`; saving a newly configured *non-default* machine did nothing, leaving its shot history to wait for the next scheduled sync tick. Every successful machine save now fires a sync immediately: `syncShots()` for the default machine, `syncMachineShots()` for any other one. Closes #729
- **New machines with a large backlog of unsynced shots now show an import progress indicator** next to the shot counter (e.g. "Import 12/40") while the initial backfill runs, instead of leaving the UI silent for however long the import takes. Only shown once a backfill has more than 5 shots to fetch; disappears automatically when it finishes. Closes #729
