...
### Added
- **Settings → Machines' Add/Edit form now has a "Save & test" button** alongside the existing standalone "Save" and "Test connection" buttons — saves the machine (create or update, same logic the plain Save button already uses) and, only if that succeeds, immediately runs the connection test against the newly-known id, showing the result inline without a second click. Previously, testing a brand-new machine required Test → "save first" error → Save → Test again, since `testMachineForm()` needs an already-assigned machine id. The plain "Save" button (save + close the form + reload the list) and the plain "Test connection" button (still requires an already-saved machine) are both unchanged — "Save & test" leaves the form open on success so the test result stays visible. Closes #727
