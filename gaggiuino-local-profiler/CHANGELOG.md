...
### Fixed
- **Flavor wheel: overlapping depth-1 labels when two adjacent top categories were both lit.** Follow-up to #299/#302/#303's depth-1 label fixes (horizontal orientation, two-line wrap for long names). When two neighboring categories were lit at once — e.g. a bean with flavors from both "Süß" and "Nussig / Kakao" — the wrapped two-line text of one could visibly overlap the other's, since ECharts' global `label:{minAngle:8, hideOverlap:true}` (`renderFlavorWheel()`, `public-src/components/flavor-wheel.js`) doesn't reliably catch collisions introduced by `overflow:'break'` line-wrapping. Reduced depth-1 label `fontSize` from 13 to 11 (`toSunburstData()`) — verified via Playwright against the real rendered zrender canvas (measuring each label's actual on-screen bounding box, not just eyeballing a screenshot) across 2, 4, and all 9 simultaneously-lit top categories: no bounding-box overlap in any case, versus a confirmed ~4px overlap before the fix. `public-src/components/flavor-wheel.js`. Closes #304

### Changed
- **Flavor wheel: replaced leader lines with color-swatch labels, matching the real SCA/WCR poster.** Comparing the leader-line labels from #299/#302 against the real SCA/WCR flavor wheel poster showed the poster has no connector lines at all — label-to-segment association there is purely by color: a small colored square sits directly before each label's text. `renderLeaderOverlay()` (`public-src/components/flavor-wheel.js`) no longer draws the `<polyline>` leader line or the small anchor `<circle>`; instead a `<rect>` swatch (filled with the segment's own color, `src.color`) is rendered directly before each label's text, moving together with it as one unit to the position computed by the unchanged angle-based collision layout (`layoutLeaderLabels()`, `public-src/flavor-wheel-labels.js`). The swatch sits on the side of the text nearer the wheel's center — leading (before the text) for right-hemisphere labels (`text-anchor:'start'`), trailing (after the text) for left-hemisphere labels (`text-anchor:'end'`), leading for centered labels near 12/6 o'clock — so reading order stays natural in both hemispheres. Horizontal container-edge clamping now accounts for the swatch's width alongside the measured text width so the whole label+swatch block never gets clipped. The angle-based collision-avoidance layout itself and the horizontal, wrapping depth-1 labels from the previous round are unchanged. `public-src/components/flavor-wheel.js`. Closes #303

### Added
- **Flavor wheel: angular label distribution + horizontal depth-1 labels.** Follow-up to #299's leader-line labels, based on two issues confirmed in a live demo. (1) `layoutLeaderLabels()` (`public-src/flavor-wheel-labels.js`) previously split labels into a fixed left/right hemisphere by wedge angle and stacked them vertically within each column — when a bean's active flavors happened to cluster in one quadrant (a common case), every label piled into a single column while the rest of the circle stayed empty. It's rewritten as a circular relaxation: each label claims an angular half-width (from its measured text width at the shared target radius) and, on collision, is nudged sideways along the arc away from its neighbor (with wraparound handling at ±π) rather than collapsed into a column — labels now spread around whatever arc they actually cluster in. `renderLeaderOverlay()` (`public-src/components/flavor-wheel.js`) computes `labelX`/`labelY` from the resolved angle at the target radius instead of a fixed column x + relaxed y, picks `text-anchor` (`start`/`end`/`middle`) from the resolved angle's cosine so labels near 12/6 o'clock center instead of defaulting to one side, and the leader line's elbow still anchors to the wedge's *original* angle so a label that had to move visibly kinks toward its final position. (2) Depth-1 category labels ("Süß", "Nussig / Kakao", etc.) turned out to still be tangentially rotated (ECharts' sunburst defaults every level to `rotate:'radial'` unless overridden — the previous assumption that omitting `rotate` meant horizontal was wrong), making longer names like "Nussig / Kakao" nearly unreadable for wedges away from 12/6 o'clock. `levels[1]` in `renderFlavorWheel()` now sets `label:{rotate:0, overflow:'break', width:64}` — labels are always horizontal and long names wrap onto a second line instead of clipping or squeezing. `public-src/flavor-wheel-labels.js`, `public-src/components/flavor-wheel.js`, `test/flavor-wheel-labels.test.js`. Closes #302
- **Flavor wheel: leader-line labels for depth 2/3, fixing label collisions on multi-flavor beans.** Depth 2/3 (mid/outer ring) labels used to render radially rotated inside their own wedge (`label.rotate: 'radial'`, `show: !!lit`); when a bean had several simultaneously matched neighboring flavors (parent+child, siblings under the same subcategory, or 3+ at once) their in-wedge text collided, and which flavors collided was unpredictable per bean since only lit nodes get labels at all. Native depth 2/3 labels are now suppressed (`show:false`; depth 1's 9 top categories are untouched — wide wedges that practically never collide) in favor of a transparent SVG overlay drawn directly over the ECharts canvas: text is moved out past the ring edge, horizontal (not rotated), connected to its wedge by a thin leader line with a small color-coded anchor dot, same idea as labels on a real pie/donut chart. New `layoutLeaderLabels()` (`public-src/flavor-wheel-labels.js`, pure and unit-tested) does generic collision avoidance — split into left/right hemisphere by the wedge's mid-angle, then a two-pass sorted relaxation per hemisphere so no two labels land closer than a minimum gap — so it works for any combination of active flavors, not just specific known-colliding pairs. Leader-line text color is now contrast-checked against the modal background (`contrastTextColor(bgHex)`) rather than the segment's own fill, since the text no longer sits on the wedge itself. Wedge geometry for the overlay is read straight from ECharts' actual computed sunburst layout (`series.getData().tree.root` + each node's `getLayout()`), not re-derived/estimated, so the lines always point at the real rendered position, including after zooming into a branch (`zoomTo()` now recomputes the overlay on every zoom). `public-src/flavor-wheel-labels.js` (new), `public-src/components/flavor-wheel.js`, `public-src/style.css`, `test/flavor-wheel-labels.test.js` (new). Closes #299
- **Installable PWA, rebuilt with server-side Ingress gating.** PWA support (manifest + service worker for "Add to Home Screen" / standalone install) was removed entirely in v1.102.1 after the v1.102.0 service worker broke the live shot graph in the HA Companion App, which loads GLP through HA Ingress inside an embedded WebView. This version adds it back with a structural fix instead of relying on client-side detection: `server.js` already reliably tells genuine Ingress requests apart from direct/LAN requests (Supervisor-internal IP + `X-Ingress-Path` header, the same check the API-token auth bypass uses); a new `isIngressRequest()` helper and an `index.html`-serving route (ahead of `express.static`) now use it to inject `<link rel="manifest">` only for non-Ingress requests. `main.js` only calls `navigator.serviceWorker.register()` when that manifest link is actually present in the page, so the Companion App's WebView — which never receives the link — never registers the service worker at all; the bug class that broke production last time cannot recur through this path. The service worker itself (`public-src/public/sw.js`) is deliberately minimal: it caches only the app shell (the document + built `/assets/` bundles), network-first with cache fallback so updates always win when online, and returns immediately without touching the cache for anything under `/api/` — the exact request pattern (`/api/system/status` polling for the live view) the old SW's interception is suspected to have broken. New `test/pwa-gating.test.js` boots the real server against a throwaway data dir and issues genuine HTTP requests (not mocked req/res) to prove the gating end-to-end. `server.js`, `public-src/main.js`, `public-src/public/manifest.json` (new), `public-src/public/sw.js` (new), `test/pwa-gating.test.js` (new). **Not yet verified against the real HA Companion App/Ingress proxy — pending manual confirmation before release.** Closes #297

### Fixed
- **Mobile: large empty-looking area between the tab bar and the shot header.** The shot header (`.sprofiler-header`, `public-src/index.html`) lays out its title block and its score-badge/export-buttons block as two `display:flex` children with `flex-wrap:wrap` on the parent — but the title block uses `flex:1;min-width:0`, which lets it shrink almost to nothing instead of ever triggering a wrap. On narrow (~390px) viewports the title/subtitle text got squeezed into a ~60px-wide sliver and visually overlapped the score badge and export buttons rather than dropping to its own row, so that ~106px-tall header region rendered as mostly illegible/overlapping text — reading as a big empty dark gap directly under the tab bar. The `@media (max-width: 768px)` block now forces both `.sprofiler-header` children to `flex: 1 1 100% !important`, so the title always gets the full row width and the score/buttons row wraps cleanly onto its own line below it. Desktop/tablet (>768px) layout is untouched. `public-src/style.css`. Closes #301
