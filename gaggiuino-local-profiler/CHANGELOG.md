...
### Added
- **Settings → Machines' "Test connection" button now saves the machine first, then tests it** — a brand-new (not-yet-saved) machine can be tested with one click instead of hitting a "save first" error. On success the result is shown inline briefly, then the form closes and the machines list reloads automatically, matching the plain "Save" button's behavior. The separate "Save & test" button introduced in an earlier unreleased change is gone again — its "leave the form open" behavior turned out to be the wrong default once tried live. **Live-testing follow-up (#731): that implicit save no longer triggers a shot import** — only an explicit "Save" click does (see the next entry); "Test connection" now saves with a `?sync=0` marker the server checks before firing the catch-up sync, so testing a connection never has an import as a side effect. Closes #729, #731
