...
Desktop shows three columns: a collapsible **navigation rail** on the far left (Shots / Live / Bibliothek / Statistiken / Bezugslog / Wartung / Bestellungen, a separator, then Einstellungen — each with an inline icon), the shot list panel, and the content area. The rail collapses to icon-only via the toggle at its top (state persisted in `localStorage`, restored on reload); the shot list panel has its own separate collapse button, independent of the rail's.

On narrow viewports (phones, portrait tablets) the rail is hidden entirely and a **bottom navigation bar** takes over: Shots / Live / Bibliothek / Statistiken, plus a "Mehr" sheet collapsing Bezugslog / Wartung / Einstellungen / Bestellungen — every entry has an inline icon, no emoji anywhere in the nav chrome. The shot list is the primary "Shots" screen, not a drawer/overlay: tapping a shot pushes to its detail view, with a back chevron in the topbar returning to the list. The multi-machine switcher lives in a small topbar above the content area on every viewport (not on a horizontal tab row). Buttons and controls that are hover-revealed on desktop (e.g. the shot-list compare button, the star rating) get a full 44×44px touch target on touch devices instead. Two things are not yet mirrored into the mobile layout: pending-count badges (Wartung/Bestellungen) don't show up on the "Mehr" sheet entries, only the section's visibility is gated the same way as on desktop; and the Live tab's pulsing "shot in progress" indicator isn't mirrored onto the bottom-nav icon, which only shows or hides.
