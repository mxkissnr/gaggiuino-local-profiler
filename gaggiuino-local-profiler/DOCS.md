...
On narrow viewports (phones, portrait tablets) the horizontal tab row is replaced by a **bottom navigation bar**: Shots / Live / Bibliothek / Statistiken, plus a "Mehr" sheet collapsing Bezugslog / Wartung / Einstellungen / Bestellungen. Tapping "Shots" opens the shot list as its own screen instead of a permanently-docked sidebar column, so the verdict header, Recipe zone and full chart fit above the fold on a typical phone screen without scrolling. The topbar mode bar survives only to host the multi-machine switcher, and hides itself when there's nothing to show there. Buttons and controls that are hover-revealed on desktop (e.g. the shot-list compare button, the star rating) get a full 44×44px touch target on touch devices instead. Two things are not yet mirrored into the mobile layout: pending-count badges (Wartung/Bestellungen) don't show up on the "Mehr" sheet entries, only the section's visibility is gated the same way as on desktop; and the Live tab's pulsing "shot in progress" indicator isn't mirrored onto the bottom-nav icon, which only shows or hides.
