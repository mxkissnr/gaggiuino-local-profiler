...
- Two consumers lose the ability to obtain a token at all once this is off, because neither has any other way to authenticate: the **installable PWA** (no token input, no client-side cache since #524) and **first-time setup of the Order Card's direct-URL mode** — an Order Card that already has `glp_token` set in its YAML from before keeps working, since that's just an `X-GLP-Token` header on a different endpoint.
...
| `expose_api_port` | Whether `GET /api/token` answers requests that don't arrive via HA Ingress (#803). Turning this off breaks the **installable PWA** and **first-time setup of the Order Card's direct-URL mode**, since neither has any other way to obtain a token — an already-configured direct-URL Order Card keeps working. Does not unmap port 8099 itself, and does not narrow the trust boundary below the whole Supervisor add-on network. See [Trust model](#trust-model) above. | `true` |
